...
const ChatBot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<{
    ...
    text: "👋 Hi there! I'm your personal Dandeli adventure guide. How can I help you plan your perfect nature getaway?",
    isUser: false
  }]);
  ...
  return <div className="fixed bottom-4 right-6 z-40">
      <Button onClick={toggleChat} className={cn("w-14 h-14 rounded-full shadow-lg flex items-center justify-center", "bg-green-600 hover:bg-green-700 transition-all duration-300", !isOpen && "animate-bounce")} aria-label="Chat with us">
        {isOpen ? <X size={24} /> : <MessageCircle size={24} />}
      </Button>

      <div className={cn("absolute bottom-20 right-0 w-80 bg-white rounded-lg shadow-xl overflow-hidden transition-all duration-300 transform", "flex flex-col border border-border", isOpen ? "scale-100 opacity-100" : "scale-90 opacity-0 pointer-events-none")} style={{
      height: isOpen ? '400px' : '0'
    }}>
        {/* Header */}
        <div className="bg-green-600 p-4 text-white">
          <h3 className="font-bold">Need help?
        </h3>
        ...
        <div className="flex-1 p-4 overflow-y-auto flex flex-col space-y-3">
          {messages.map((message, index) => <div key={index} className={cn("max-w-[80%] p-3 rounded-lg animate-slide-up", message.isUser ? "bg-green-600 text-white self-end rounded-br-none" : "bg-gray-100 self-start rounded-bl-none")} style={{
          animationDelay: `${index * 100}ms`
        }}>
              {message.text}
            </div>)}
          {isTyping && <div className="flex space-x-2 p-3 bg-gray-100 self-start rounded-lg max-w-[80%]">
        ...
        <div className="p-3 border-t border-border flex items-center">
          <input type="text" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} placeholder="Type your message..." className="flex-1 px-3 py-2 border rounded-l-md focus:outline-none focus:ring-1 focus:ring-green-600" />
          <Button onClick={handleSend} className="rounded-l-none bg-green-600 hover:bg-green-700" disabled={input.trim() === ''}>
            <Send size={18} />
          </Button>
        </div>
      </div>
