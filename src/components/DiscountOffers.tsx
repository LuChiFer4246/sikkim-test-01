...
const OfferCard: React.FC<OfferProps> = ({
  ...
  return <motion.div className={cn("rounded-xl overflow-hidden shadow-lg hover:shadow-xl group relative", featured && "md:col-span-2 md:row-span-2")} initial={{
    opacity: 0,
    y: 20
  }} whileInView={{
    opacity: 1,
    ...
    once: true
      ...
      <div className={cn("relative", featured ? "h-96" : "h-64")}>
        ...
        {featured && <div className="absolute top-0 left-0 m-4 bg-yellow-500 text-white px-3 py-1 rounded-full text-sm font-medium">
            Featured Package
          </div>}
      </div>
      <div className={cn("p-6", backgroundColor, textColor)}>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to={linkTo} className={cn("rounded-full py-2 px-4 inline-flex items-center gap-1 bg-white/20 hover:bg-white/30 transition-all", textColor)}>
...
const DiscountOffers: React.FC = () => {
  return <section className="py-16 bg-gradient-to-br from-purple-50 to-blue-50 relative overflow-hidden" id="day-package">
